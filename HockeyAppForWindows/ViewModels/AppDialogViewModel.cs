...
namespace HockeyApp.AppLoader.ViewModels
{
    public class AppDialogViewModel:ViewModelBase
    {
        ...
        public bool IsVersionEditable { get { return this.AppInfo.Platform != AppInfoPlatforms.Android; } }
        ...
        public void ResetToDefaults()
        {
            ...
            if (!string.IsNullOrWhiteSpace(args.NotesFile))
            {
                if (File.Exists(args.NotesFile))
                {
                    StreamReader sr = new StreamReader(File.OpenRead(args.NotesFile));
                    this.Notes = sr.ReadToEnd();
                    sr.Close();
                }
                else
                {
                    this.Notes = "Notes-file not found! (argument: " + args.NotesFile + ").";
                }
            }else if (!string.IsNullOrWhiteSpace(args.Notes))
            {
